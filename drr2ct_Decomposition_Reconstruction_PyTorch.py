...
#app
def write_dicom(image2d, i, slicesLocation, filename_little_endian, dt, siuid):
...
with torch.set_grad_enabled(False):
    for i, (image_app, label_app) in enumerate(
        ...
        for j in range(0, e):
            ...
            filename_little_endian = ("/content/drive/My Drive/NewDataset/newSlices/%d.dcm" % (j + 1,))
            ds = write_dicom(t, j, slicesLocation, filename_little_endian, dt, siuid)


        u = out1[0][0].cpu().numpy()
        u = cv2.resize(u, dsize=(512, 512), interpolation=cv2.INTER_CUBIC)
        slicesLocation = float(slice1Location - (sliceThickness * 0))
        filename_big_endian = ("/content/drive/My Drive/NewDataset/newImage/1.dcm")
        ds = write_dicom(t, 0, slicesLocation, filename_big_endian, dt, siuid)



dss=pydicom.read_file("/content/drive/My Drive/NewDataset/newImage/1.dcm")
w=dss.pixel_array
dss1=pydicom.read_file("/content/drive/My Drive/NewDataset/newSlices/%d.dcm" % (slice_number,))
v=dss1.pixel_array
